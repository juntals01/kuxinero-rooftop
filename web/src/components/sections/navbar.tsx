...
export function Navbar() {
  ...
  return (
    <div className="absolute top-0 left-0 w-full z-50 py-4 lg:py-5 px-5 sm:px-8 lg:px-[120px]">
      <div className="max-w-[1440px] mx-auto">
        <nav className="relative flex items-center justify-between h-16 lg:h-[68px] px-5 sm:px-6 lg:px-10 rounded-full bg-white/[0.91] shadow-nav border border-accent/25 backdrop-blur-sm">
        ...
        {open && (
          <div className="md:hidden mt-2 rounded-2xl bg-white/95 backdrop-blur-md shadow-nav border border-accent/20 p-4 flex flex-col gap-1">
            ...
            <Link
              ...
              target="_blank"
              onClick={() => setOpen(false)}
              className="flex items-center justify-center gap-2 h-10 mt-2 rounded-full bg-accent text-primary text-sm font-semibold shadow-button"
            >
              <MapPin className="w-4 h-4" />
              Get Directions
            </Link>
